...
class Validator
{
    ...
    */
    ...
    // Método para obtener el nombre del archivo validado.
    public static function getFilename2()
    {
        return self::$filename2;
    }

    // Método para obtener el error al validar un archivo.
    public static function getFileError2()
    {
        return self::$file_error2;
    }
    ...
    */
    ...
    public static function validateImageFile2($file)
    {
        if (is_uploaded_file($file['tmp_name'])) {
            // Se obtienen los datos de la imagen.
            $image = getimagesize($file['tmp_name']);
            ...
            if ($file['size'] > 209715200000) {
                ...
                $extension = strtolower(pathinfo($file['name'], PATHINFO_EXTENSION));
                // Se establece un nombre único para el archivo.
                self::$filename2 = uniqid() . '.' . $extension;
                return true;
            } else {
                self::$file_error = 'El tipo de imagen debe ser jpg o png';
                return false;
    ...
    */
    ...
    public static function saveFile2($file, $path)
    {
        if (!$file) {
            return false;
        } elseif (move_uploaded_file($file['tmp_name'], $path . self::$filename2)) {
            return true;
        } else {
            return false;
        }
    ...
    */
    ...
    public static function changeFile2($file, $path, $old_filename2)
    {
        if (!self::saveFile2($file, $path)) {
            return false;
        } elseif (self::deleteFile2($path, $old_filename2)) {
            return true;
        } else {
            return false;
        }
    ...
    */
    ...
    public static function deleteFile2($path, $filename2)
    {
        if ($filename2 == 'default.png') {
            return true;
        } elseif (@unlink($path . $filename2)) {
            return true;
        } else {
            return false;
        }
